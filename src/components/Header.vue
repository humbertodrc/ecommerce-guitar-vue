				...
				>
					<div class="carrito">
						...
						<div id="carrito" class="bg-white p-3">
							<!-- Si el carrito esta vacio aplicamos la directiva v-if asi se mostrara: -->
							<p v-if="carrito.length === 0" class="text-center m-0">
								El carrito esta vacio
							</p>
							<!-- Si el carrito ya tiene articulos mostramos el contendido con la directiva v-if="carrito.length > 0" otra opcion es utilizar v-else -->
							<div v-else>
									...
									<tbody>
										...
										>
											...
											<td class="flex align-items-start gap-4">
												<button @click="$emit('decrementar-cantidad', producto)" type="button" class="btn btn-dark">-</button>
												{{ producto.cantidad  }}
												<button @click="$emit('incrementar-cantidad', producto)" type="button" class="btn btn-dark">+</button>
											</td>
											<td>
